...
public class ActivityPickerActivity extends ListActivity {
    ...
    private class ActivityAdapter extends ArrayAdapter<ResolveInfoWrapper> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            String packageName = " \n(";
            if(info.getInfo().activityInfo != null)
            {
                packageName += info.getInfo().activityInfo.packageName;
            }
            if(info.getInfo().serviceInfo != null)
            {
                packageName += info.getInfo().serviceInfo.packageName;
            }
            if(info.getInfo().providerInfo != null)
            {
                packageName += info.getInfo().providerInfo.packageName;
            }
            packageName += ")";

            final TextView textView = (TextView) view.getTag();
            textView.setText(info.getInfo().loadLabel(mPackageManager) +  packageName );
